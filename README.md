# AmazeMed Website

Static HTML website for AmazeMed, ready for deployment to GitHub Pages.

## Local Testing

### Option 1: Python HTTP Server (Recommended)

Python 3 comes with a built-in HTTP server. This is the simplest way to test locally:

```bash
python3 -m http.server 8000
```

Then open your browser and visit:
- `http://localhost:8000`

### Option 2: Node.js http-server

If you have Node.js installed:

```bash
npx http-server -p 8000
```

Then visit `http://localhost:8000`

### Option 3: VS Code Live Server

If you're using VS Code, install the "Live Server" extension and right-click on `index.html` to select "Open with Live Server".

## File Structure

- `index.html` - Redirects to homepage
- `home.html` - Homepage
- `about.html` - About page with company mission and co-founders
- `services.html` - Services page listing dev shop capabilities
- `contact.html` - Contact page with email information
- `404.html` - Custom 404 page with routing logic
- `styles.css` - Shared stylesheet
- `nav.js` - Shared navigation component

## GitHub Pages Setup

### 1. Create GitHub Repository

1. Go to [GitHub](https://github.com) and create a new repository
2. Name it `amazemed` (or your preferred name)
3. **Do NOT** initialize with README, .gitignore, or license (we already have these)

### 2. Push to GitHub

From your local directory:

```bash
# Initialize git (if not already done)
git init

# Add all files
git add .

# Create initial commit
git commit -m "Initial commit: AmazeMed website"

# Add GitHub remote (replace YOUR_USERNAME with your GitHub username)
git remote add origin https://github.com/YOUR_USERNAME/amazemed.git

# Push to GitHub
git branch -M main
git push -u origin main
```

### 3. Enable GitHub Pages

1. Go to your repository on GitHub
2. Click **Settings** (in the repository menu)
3. Scroll down to **Pages** (in the left sidebar)
4. Under **Source**, select:
   - Branch: `main`
   - Folder: `/ (root)`
5. Click **Save**

### 4. Access Your Site

After enabling GitHub Pages, your site will be available at:
- `https://YOUR_USERNAME.github.io/amazemed/`

GitHub Pages may take a few minutes to build and deploy. You'll see a green checkmark when it's ready.

## Route Handling

The `404.html` file handles routing for clean URLs:
- `/home` → `home.html`
- `/about` → `about.html`
- `/services` → `services.html`
- `/contact` → `contact.html`

GitHub Pages will automatically use `404.html` for any routes that don't match existing files.

## Notes

- All HTML files are self-contained with shared CSS and JavaScript
- Files are designed to work both standalone and within GitHub Pages
- The `.gitignore` excludes Python cache files, backup folders, and virtual environments

## Updating the Site

To update the site:

```bash
# Make your changes to HTML files
git add .
git commit -m "Update website content"
git push
```

GitHub Pages will automatically rebuild and deploy your changes (usually within 1-2 minutes).

# amazemed_website
